...
def make_model(scale, upsampler, input_dim, config=None):
    ...
    return SwinIR(upsampler=upsampler, upscale=scale, img_size=img_size, input_dim=input_dim)
    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                ...
                m.append(common.PixelShuffle1D(2))
        elif scale == 3:
            m.append(nn.Conv2d(num_feat, 3 * num_feat, 3, 1, 1))
            m.append(common.PixelShuffle1D(3))
        else:
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        ...
        m.append(common.PixelShuffle1D(scale))
...
class SwinIR(nn.Module):
    ...
    # Note that this setting is lite version of SwinIR
    def __init__(
        self,
        img_size=128,
        input_dim=1,
        ...
        **kwargs,
        ...
        self.conv_first = nn.Conv2d(input_dim, embed_dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.upsampler == "none":
            ...
            # for image denoising and artifact reduction
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_last(x)
